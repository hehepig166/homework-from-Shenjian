...
//		特别测试：打开下面这行的注释，期望编译器会报错，如果不报错，说明定义有问题，不符合预期的语义（为什么？仔细想!!!）
	...
	if (1) {
		TStringAdv s1("******"), s2 = "abc";
		const char* str3 = "def", str4[] = "xyz";

		cout << "insert简单测试(TStringAdv类和TStringAdv类、字符指针、字符数组、字符)" << endl;

		s1 = "******";
		s1.insert(str4, 1);
		cout << "s1应为xyz******，实际输出：" << s1 << endl;

		s1 = "******";
		s1.insert(str3, 7);
		cout << "s1应为******def，实际输出：" << s1 << endl;

		s1 = "******";
		s1.insert(s2, 4);
		cout << "s1应为***abc***，实际输出：" << s1 << endl;
	...
	if (1) {
		...
		cout << "串应为This is a red pencil，实际输出：" << s1.substr(1) << endl;

		cout << "串应为This is a red pencil，实际输出：" << s1.substr(1, 1000) << endl;

		cout << "串应为This，实际输出：" << s1.substr(1,4) << endl;

		cout << "串应为is a red pen，实际输出：" << s1.substr(6, 12) << endl;

		cout << "串应为<NULL>，实际输出：" << s1.substr(0, 999) << endl;

		cout << "串应为<NULL>，实际输出：" << s1.substr(999, 0) << endl;
