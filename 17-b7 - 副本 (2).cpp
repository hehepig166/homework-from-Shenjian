    ...
    while ((entry = readdir(dp)) != NULL) {
        ...
        if (S_ISDIR(statbuf.st_mode)) {
            if (strcmp(".", entry->d_name) != 0 && strcmp("..", entry->d_name) != 0) {
                searchAllFiles(entry->d_name, testinfo, stuinfo);
            }
        }
        else {
            getchar();
            touchOneFile("", entry->d_name, testinfo, stuinfo);
            flag = 1;
        }
    }
...
#define FAIL_ANALYSE_FIRSTLINE toks.release(); return "未取到首行信息"
    ...
    while (1) {
        ...
        while (!tmpstr.empty()) {
            if (f == 0) {
                int is1 = 0, is2 = 0;
                unsigned int pos1(tmpstr.find("/*")), pos2(tmpstr.find("//"));
