...
void TString::rplus(const char *str)
{
    //无效的str，不进行操作（还是原串）
    if (str == NULL || *str == '\0') {
        return;
...
void TString::lplus(const char *str)
{
...
int TString::find_baoli(const char *T, const int Tlen) const
{
...
int TString::find_KMP(const char *T, const int Tlen, int *nxt) const
{
    //calc_nxt
    nxt[0] = -1;
...
void TString::del(const char *str)
{
    //无效串
    if (str == NULL || *str == '\0') {
        return;
...
char *TString::multiply(const int n) const
{
...
void TString::exitERR(const char *hint) const
{
    puts(hint);
    getchar();
    exit(-1);
